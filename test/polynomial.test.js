const pFactory = require('../src/jqrPoly.js');
const assert = require('assert');

describe('polynomial', () => {
	describe('isZero()', () => {
		it('isZero(1X^2 + 3x^1 + 2x^0) --> false', () => {
			let poly = pFactory.create([1, 3, 2]);
			assert.equal(poly.isZero(), false);
		});
		it('isZero(0x^0) --> true', () => {
			let poly = pFactory.create([0]);
			assert.equal(poly.isZero(), true);
		});
		it('isZero(0X^2 + 0x^1 + 0x^0) --> true', () => {
			let poly = pFactory.create([0, 0, 0]);
			assert.equal(poly.isZero(), true);
		});
	});
	describe('evaluate()', () => {
		it('1X^2 + 1x^0 evaluate(0) --> 1', () => {
			let poly = pFactory.create([2, 0, 1]);
		...
		it('1X^2 + 0X^1 + 1x^0 evaluate(1) --> 2', () => {
			let poly = pFactory.create([1, 0, 1]);
		...
		it('1X^4 + 2X^3 + 2X^2 + 1x^1 + 0x^0 evaluate(2) --> 42', () => {
			let poly = pFactory.create([1, 2, 2, 1, 0]);
	...
	describe('multiplyScalar()', () => {
		...
		it('should return zero when parameter is zero', () => {
			let poly = pFactory.create([1, 0, 2]);
	...
	describe('divide()', () => {
		...
		it('(x^3 - 2x^2 -4) / (x -3) = (x^2 + x + 3 | 5)', () => {
			let p1 = pFactory.create([1, -2, 0, -4]);
			let p2 = pFactory.create([1, -3]);
		...
		it('(x^5 + 2x^3 − x − 8) / (x^2 − 2^x + 1) = (x^3 + 2x^2 + 5^x + 8 | 10x − 16)', () => {
			let p1 = pFactory.create([1, 0, 2, 0, -1, -8]);
			let p2 = pFactory.create([1, -2, 1]);
			let [quotient, remainder] = p1.divide(p2);
